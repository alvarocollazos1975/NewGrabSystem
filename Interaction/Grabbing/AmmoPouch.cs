...
namespace BIMOS
{
    public class AmmoPouch : Grab
    {
        ...
        public override void OnGrab(GrabHandler hand) //Triggered when player grabs the grab
        {
            ...
            magazine.transform.SetPositionAndRotation(hand._hand.PhysicsHandTransform.position, hand._hand.PhysicsHandTransform.rotation);

            foreach (Grab grab in magazine.GetComponentsInChildren<SnapGrab>())
                if (grab.IsLeftHanded && hand._hand.IsLeftHand || grab.IsRightHanded && !hand._hand.IsLeftHand)
                {
                    grab.OnGrab(hand);
                    break;
                }
